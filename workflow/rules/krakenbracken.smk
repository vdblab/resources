...
all_bracken_dbs= expand(
    f"{dbs}/kraken/zymogutv0/database{{readlens}}mers.kmer_distrib",
...
rule make_bracken_dbs_for_zymo_mock:
    ...
    input:
        f"{dbs}/kraken/zymogutv0/hash.k2d"
    output:
        f"{dbs}/kraken/zymogutv0/database{{readlen}}mers.kmer_distrib",
    threads: 8
    shell: """
    DB=$(dirname {output})
    # see
...
rule complete_bracken_db:
    ...
    container: "docker://ghcr.io/vdblab/kraken2:2.1.2",
    input:
        expand(f"{dbs}/kraken/zymogutv0/database{{readlen}}mers.kmer_distrib",
